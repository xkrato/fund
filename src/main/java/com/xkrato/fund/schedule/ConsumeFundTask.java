...
public class ConsumeFundTask {
    ...
    private void sendEmail(Fund fund) {
        Email from = new Email(SEND_GRID_EMAIL_SENDER);
        String subject = SEND_GRID_SUBJECT_FUND;
        Email to = new Email(SEND_GRID_EMAIL_RECEIVER);
        Content content = new Content("text/plain", SEND_GRID_CONTENT_FUND + fund.getId() + " - " + fund.getManagerName());
        Mail mail = new Mail(from, subject, to, content);

        SendGrid sg = new SendGrid(DESedeUtil.decrypt(DES_KEY, SEND_GRID_API_KEY));
